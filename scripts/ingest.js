...
async function main() {
  ...
  for (const pdfPath of pdfPaths) {
    ...
    catch (e) { console.error(`   ⚠️  Cannot read file: ${e.message}`); continue; }

    let text;
    try { const parsed = await pdf(dataBuffer); text = parsed.text; }
    catch (e) { console.error(`   ⚠️  PDF parse failed: ${e.message}`); continue; }

    text = text.replace(/\x00/g, '').replace(/\f/g, '\n').replace(/[ \t]{3,}/g, '  ').replace(/\n{4,}/g, '\n\n').trim();

    if (text.length < 100) { console.log('   ⚠️  Extracted text too short — may be scanned. Skipping.'); continue; }
    ...
    for (let b = 0; b < chunks.length; b += BATCH_SIZE) {
      ...
      catch (e) { console.error(`   ⚠️  Embedding failed: ${e.message}`); continue; }
